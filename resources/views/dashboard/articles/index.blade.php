...
<div class="content">
	...
	<div class="card">
		<div class="card-header d-flex align-items-center py-0 ">
			...
			<div class="ms-3">
				<a type="button" href="{{ url('dashboard/articles/create') }}" class="btn btn-indigo"><i class="ph-plus me-1"></i> Create New</a>
			</div>
			<div class="ms-2">
				<div class="wmin-250">
					<select id="switchStatus" class="form-control form-control-select2">
						<optgroup label="Articles Status">
...
$(document).ready(function() {
	...
	$('#datatable-news tbody').on( 'click', '#update', function () {
        var data = table.row( $(this).parents('tr') ).data();
		var id = data['id'];	
			window.location.href = BaseUrl+"/dashboard/articles/update/"+id;
    });
	
	$('#datatable-news tbody').on( 'click', '#approved', function () {
        var data = table.row( $(this).parents('tr') ).data();
	...
	$('#switchStatus').on( 'change',function () {
		...
		if ( ! $.fn.DataTable.isDataTable( '#datatable-news' ) ) {
			loadNews();
		}else{
